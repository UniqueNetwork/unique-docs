...
Note that some bounds are also set by the global chain limits (see `setChainLimits`). The more restrictive limits will always apply. 
    ...
    * `sponsor_timeout` - Time interval in blocks that defines once per how long a non-privileged user transaction can be sponsored. Default value is 14400 (24 hrs), allowed values are from 0 (not limited) to 10,368,000 (1 month). 
